...
Shader "PDT Shaders/SelectorGrid" {
	Properties {
		_LineColor ("Line Color", Color) = (1,1,1,1)
		_CellColor ("Cell Color", Color) = (0,0,0,0)
		_SelectedColor ("Selected Color", Color) = (1,0,0,1)
		[PerRendererData] _MainTex ("Albedo (RGB)", 2D) = "white" {}
		[IntRange] _GridSizeX("Grid Size", Range(1,100)) = 10
		[IntRange] _GridSizeY("Grid Size", Range(1,100)) = 10
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			if (frac(uv.x*gsizex) <= _LineSize)
			{
				brightness = _LineColor.w;
				color = _LineColor;
			}
			if (frac(uv.y*gsizey) <= _LineSize)
			{
				brightness = _LineColor.w;
				color = _LineColor;
			}
